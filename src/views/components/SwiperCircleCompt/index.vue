<template>
  <div class="ctx" :style="{width:width,height:height}">
    <div id="main">
      ...
      <el-button type="text" class="bnt-arrow" @click="onPrevItem">
        <i class="el-icon-arrow-left" />
      </el-button>
      <div id="content">
        <div v-for="(item,index) in lstData" :key="'artist'+index" class="nft-item" @click="onView(item.address)">
          <!-- <el-image class="nft-image" fit="contain" :src="item.avatar?($baseUrl +item.avatar):require('@/assets/images/logo2.jpg')" /> -->

          <div class="flex-row flex-hvc">
            <el-avatar :src="`${baseUrl}${item.avatar}`" class="wdt-120 hgt-120" />
          </div>
          <h3 class="txt-ellipsis dispaly-block wdtp-100 txt-algin-l hgt-38">{{ item.real_name }}</h3>
          <h4 class="txt-ellipsis-more txt-wrap wdtp-100 hgt-101 txt-algin-l pdlr-2">简介:<br>{{ item.description }}</h4>
        </div>
      </div>
      ...
      <el-button type="text" class="bnt-arrow" @click="onNextItem">
...
export default {
  ...
  methods: {
    ...
    onView(address) {
      // this.$router.push({ path: `/my/intro?address=${address}` })
...
.bnt-arrow{
  border: none;
    ...
    color: #FFFFFF;
    &:hover{
      background-color: rgba(84, 109, 137, 0.11);
    }
